...
public interface ClienteRepository extends JpaRepository<Cliente, Long>{
	...
	// Starting e Ending
//	List<Cliente> findByNomeStartingWidth(String nome);
...
//	List<Cliente> findByEndereco_CepContaining(String cep);
	...
	Pageable pageble = PageRequest.of(1, 5, Sort.by(Sort.Direction.ASC, "email"));
	Page<Cliente> findAll(Pageable pageable);
	
	// Página atual; cliente.getNumber()
	// Total de itens da consulta: Clientes.getTotalElemnts()
	
	// === JPA Queries ===
	@Query("SELECT c FROM Cliente c WHERE c.nome = :nomeCliente")
	List<Cliente> findAllByNomeCliente(@Param("nomeCliente") String nomeCliente, Pageable pageable);
	
	@Query("SELECT c FROM Cliente c WHERE c.email = :emailCliente")
	List<Cliente> findAllByEmailCliente(@Param("emailCliente") String email, Pageable pageable);
	
	// query usando nome e data (apenas exemplos)
	/*
	 *  DateTimeFormatter formataador = DataTimeFormatter.ofPattern("dd/MM/yyyt");
	 *  LocalDate data = LocalDate.parse("17/02/2024", formatador)
	 */
//	@Query("SELECT c FROM Cliente c WHERE c.nome = :nome AND c.dataCadastra = :data")
//	List<Cliente> findAllByEmailDoCliente(@Param("nome") String nome, @Param("data") LocalDate data);
	...
	// parém no Spring Data, não precisamos criar a classe, criamos apenas a interface
	@Query("SELECT c.id, c.nome, c.email FROM Cliente c")
	List<ClienteProjection> findClienteProjecao();
	
	// === JPQL Named Queries
	/*
	 * @NamedQuery(name = "Cliente.buscarPF", query = "SELECT c FROM Cliente c WHERE c.tipoPessoa='Fisica' AND c.cpfOuCnpj = :cpf)
	 * @Entity
	 * public class Cliente {
	 * ....
	 */
	// o nome do método deve ser o nome da named querty!!
